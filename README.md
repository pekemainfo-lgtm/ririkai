...
このREADMEはPhase 3（カレンダー：月間表示・日単位集計・セッション一覧・セッション詳細・Markdown表示）まで完了時点の内容。
...
| `web/session.html` | 学習セッションの入力・保存・AI分析 |
...
生の文字起こしは含まない。分析結果（理解できている点・曖昧な点・誤解の可能性・確認質問・復習事項）と、MarkdownのS3キーを保持し、一覧・詳細表示を高速に行えるようにしている。
...
- カード生成・重複統合（conceptKey / cardType / 正規化質問文）はPhase 4・5で実装予定（SPEC.md §10, §22）
...
## 現在未実装の機能（Phase 3完了時点）

- ノート写真アップロード（Phase 7）
- カード生成・カード候補確認・重複統合（Phase 4・5）
- 復習機能（Phase 6）
- カレンダーの新規カード数・復習予定数・要確認数（カード/復習機能実装後に対応）
